const mainApp = () => {
	...
	function handleBttnClick(this: HTMLButtonElement) {
		...
		if (this.classList.contains('operator')) {
			...
			if (decimalBttn) decimalBttn.disabled = false
		} else if (this.classList.contains('operand')) {
			if (bttnValue === '.') {
				if (decimalBttn) decimalBttn.disabled = true
			}
	...
	function plusMinusFN() {
		...
		let newTempDisplayValue: string
	...
	function solveFN(): string {
		...
		if (num2 === 0 && _operator === '/') result = 'Halt, Citizen! Illegal Operation!'
